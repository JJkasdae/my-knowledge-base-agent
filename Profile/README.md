...
- **Starts empty, and stays out of git.** Every other content folder ships with example material; this one cannot. `.gitignore` excludes everything here except this README, under the heading *Personal record*, because names, employers, education, and visa status do not belong in a saved history that might one day get pushed somewhere. Your notes are versioned. Your personal record deliberately is not. Add the first entry whenever you want one.
